...
| `components/wms/dashboard.tsx` | Логотип в шапке кабинета, `font-serif` на заголовке раздела |
...
1. **Личный кабинет здесь (в v0) содержит только 4 раздела** (Заказы,
   Профиль, Клиенты, Сборщик). На реальном slotwms.ru в кабинете **7
   разделов**: Заказы, Профиль, Клиенты, Товары, Склад, План, Заявки.
   Дизайн-система (токены, компоненты, шрифты) полностью применима к
   недостающим разделам — их нужно доверстать по этому же стилю.
...
- [ ] Доверстать недостающие разделы кабинета (Товары, Склад, План, Заявки)
