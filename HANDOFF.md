...
## 4.1 Текстовый контент заменён на реальный (снят со slotwms.ru)

Ранее в этом проекте использовался **придуманный текст** (мультимаркетплейс
Wildberries/OZON/Яндекс Маркет, тариф «первый адрес бесплатно + 2 900 ₽»).
Это было ошибкой — контент теперь снят напрямую со страницы
`https://slotwms.ru` (через живой снапшот DOM) и заменён везде:

- **Позиционирование**: реальный продукт — это WMS именно для **Wildberries**
  (не мультимаркетплейс). H1 реального сайта: «WMS, которая снижает ошибки
  сборки до нуля». Ключевая фича — работа с **Честным ЗНАКом (КИЗ)**, которой
  не было в старой версии контента.
- **Тарифы** (реальные, 3 плана вместо старых 2):
  - Старт — **8 999 ₽/мес**, до 3 сотрудников
  - Стандарт — **50 000 ₽/мес**, до 15 сотрудников
  - Корпоративный — **100 000 ₽/мес**, без ограничений
- **Реквизиты**: реальный сайт работает как **ИП** (не ООО). Точный ИНН/ОГРНИП
  на странице не отображается открыто — в футере оставлен явный placeholder
  `[уточнить]`, который нужно заполнить вручную (см. п. 7.5) — **не берите
  цифры из старой версии, там был вымышленный ИНН ООО**.
- **Новые секции**, добавленные на лендинг по образцу реальной структуры:
  `pain-points.tsx` («Боли» — 6 карточек проблем), `solution.tsx`
  («Решение» — 3 фичи + список результатов), `case-study.tsx` (кейс с
  цифрами +3× / −90% / 1 час + отзыв).
- Интеграции (`integrations.tsx`) сокращены до того, что реально есть:
  **Wildberries, Честный ЗНАК, Android** — убраны OZON/Яндекс
  Маркет/СберМегаМаркет, которые были придуманы ранее и не соответствуют
  реальному продукту.
- Обновлены под новый фокус: `app/layout.tsx` (title/description/keywords/OG),
  `public/llms.txt`, `seo/site-structure.json` (запланированные страницы
  `/wms-ozon` и `/wms-yandex-market` заменены на `/wms-wildberries` и
  `/chestnyy-znak`), `faq.tsx`, `final-cta.tsx`, `site-nav.tsx` (пункты меню
  и якоря `#pains`/`#solution`/`#case`).

**Логотип не менялся** — по вашей просьбе оставлен как есть (см. п. 4).
...
| `components/landing/pain-points.tsx` | **Новый компонент** — секция «Боли» (6 карточек: ошибки сборки, нет учёта, КИЗы, путаница с заказами, склад в мессенджерах, не масштабируется) |
| `components/landing/solution.tsx` | **Новый компонент** — секция «Решение» (3 фичи + блок «Что получаете») |
| `components/landing/case-study.tsx` | **Новый компонент** — секция «Кейсы» с цифрами (+3× / −90% / 1 час) и отзывом |
| `components/landing/pricing.tsx` | Тарифы заменены на реальные 3 плана (8 999 / 50 000 / 100 000 ₽), сетка 2→3 колонки |
| `components/landing/integrations.tsx` | Список сокращён до реальных интеграций: Wildberries, Честный ЗНАК, Android |
| `components/landing/hero.tsx`, `stats.tsx`, `site-nav.tsx`, `faq.tsx`, `final-cta.tsx`, `site-footer.tsx` | Текст обновлён на реальный контент со slotwms.ru (см. п. 4.1) |
| `app/layout.tsx` | Метатеги (title/description/keywords/OG) и `organizationSchema` обновлены под реальный фокус на Wildberries |
| `public/llms.txt`, `seo/site-structure.json`, `app/sitemap.ts` | Синхронизированы с реальным контентом и тарифами (см. п. 4.1) |
...
Запланированные страницы (`/wms-wildberries`, `/chestnyy-znak`,
`/features`, `/pricing`, `/blog`) — в том же JSON есть готовые
title/description/h1/h2/keywords/schema для каждой; создайте их как
`app/<slug>/page.tsx`, используя `SiteNav`/`SiteFooter`.
...
2. Логотип — временная реконструкция (см. п. 4), **в этой сессии не менялся
   по прямой просьбе — оставлен как есть**.
3. **Юридические реквизиты в футере — `[уточнить]`**. Реальный сайт работает
   как ИП (Курдин Никита Владимирович), но точный ИНН/ОГРНИП не был указан
   открыто на странице, поэтому оставлен явный placeholder вместо того,
   чтобы придумывать цифры — впишите ИНН и ОГРНИП из своих документов
   в `components/landing/site-footer.tsx`.
...
- [ ] Заполнить ИНН и ОГРНИП ИП в футере вместо `[уточнить]`
- [ ] Заменить AI-сгенерированные фото (`warehouse-hero.png`, `gallery-*.png`) на реальные фото склада
- [x] Заменить придуманный текст/тарифы на реальный контент со slotwms.ru (тарифы, боли, решение, кейс, интеграции)
- [ ] Сверить окончательные цифры тарифов и лимиты сотрудников с актуальным прайсом (могли измениться после снятия снапшота)
