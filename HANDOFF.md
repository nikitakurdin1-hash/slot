...
| `components/landing/warehouse-gallery.tsx` | **Новый компонент** — секция «Реальный склад» на лендинге: бенто-галерея из 4 фото (терминал сбора данных / стеллажи / упаковка / отгрузка), подключена в `app/page.tsx` между `HowItWorks` и `Features` |
| `public/gallery-scanner.png`, `gallery-racks.png`, `gallery-packing.png`, `gallery-shipping.png` | **Новые изображения** — сгенерированы AI, требуют замены на реальные фото склада перед продом (см. п. 7.4) |
...
4. **Все фото на лендинге (`warehouse-hero.png` и 4 файла `gallery-*.png`)
   сгенерированы AI**, это не реальные фото складов slotwms.ru. Визуально
   похожи на профессиональную фотосъёмку складского комплекса, но перед
   продом их нужно заменить на настоящие фото вашего склада/операций —
   иначе есть риск, что клиенты узнают несоответствие с реальностью.
...
- [ ] Заменить AI-сгенерированные фото (`warehouse-hero.png`, `gallery-*.png`) на реальные фото склада
